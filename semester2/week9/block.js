...
//     this.blue = map(mouseY, 0, height, 255, 0);
 ...
 this.applyForce(ali);
  ...
  sizing() {
    ...
    if (gohome == false) {
      this.sizerr = map(this.rgb.x,0,255, 0, 4);
     // this.alpha = floor((this.rgb.x + this.rgb.y +this.rgb.z)/6);
    }else{
      this.sizerr = map(this.sizerr, 0, 4, this.sizerr, 1);
      //this.alpha = 255;
    }
    ...
    this.alpha = this.rgb.x/6;//floor((this.rgb.x + this.rgb.y +this.rgb.z)/12);
     ...
     rect((this.position.x), this.position.y, this.size, this.size);
      ...
      //GREEN
      stroke(this.rgb.x, this.rgb.y/2,0,this.alpha);
      fill(this.rgb.x, this.rgb.y/2,0,this.alpha);
      rectMode(CENTER);
     //rect(this.position.x,this.position.y,this.size,this.size);
      rect(this.position.x, this.position.y, (this.size)*this.sizerr, this.size * this.sizerr);
    //
    //   //BLUE
