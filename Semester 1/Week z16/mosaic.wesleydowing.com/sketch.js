...
function loadingScreen() {
  ...
  //pg.rect(width / 2, height / 2 - 20, ellsize);

 // pg.tint(255,alpha_load);
 pg.image(capture,0,0,width,height);
  // pg.image(capture,width/2-(ellsize/2),height/2-(ellsize/2),ellsize,ellsize);
//pg.noTint();
    ...
    if (isMobile == false) {
      ...
      for (var i = (start_width); i < (end_width); i += skip) {
        for (var j = start_height; j < (end_height); j += skip) {
          ps.addParticle((x + i) / factor, (y + j) / factor, 0, bg);
        }
      }
    } else {
