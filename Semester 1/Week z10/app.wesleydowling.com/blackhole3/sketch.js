...
if (isMobile == false) {
  ...
  pg.text('click/touch to move circle', width / 2, height - (height/3));
pg.text('to disperse image', width / 2, height - (height/3) + 20);

  pg.text('hold mouse button/two finger touch', width / 2, (height - (height/3))+60);
  ...
  if (mouseIsPressed == true && mouseX > (width / 2 - 30) && mouseX < (width / 2 + 30) && mouseY > (height / 2 - 30) && mouseY < (height / 2 + 30)) {
    trigger = true;
  }
}
...
function draw() {
  ...
  if (alpha_load < 255) {
    alpha_load = alpha_load + 2;

  }

  if (loading_alpha >= 0 && trigger == true) {
...
function blackhole_draw() {
  ...
  var sine = abs(1 * cos(TWO_PI * frameCount / 1600));
...
function mousePressed() {
  ...
  if (mouseX > 0 && mouseX < width && mouseY > height-40 && mouseY < height) {
    let fs = fullscreen();
    fullscreen(!fs);
  }
